...
def J_BF_inB(q):
    assert q.shape == (3, 1), "Configuration vector q must be size (3,1)"
    alpha, beta, gamma = q[0, 0], q[1, 0], q[2, 0]
    J = np.array([[0, -(np.sin(beta)*np.sin(gamma) + np.cos(beta)*(1+np.cos(gamma))), np.cos(beta - gamma)],
                         [np.cos(alpha)*(np.cos(beta-gamma)+np.cos(beta)+1), -np.sin(alpha)*(np.sin(beta)+np.sin(beta-gamma)), np.sin(alpha)*np.sin(beta-gamma)],
                         [np.sin(alpha)*(1+np.cos(beta)+np.cos(beta-gamma)), np.cos(alpha)*(np.sin(beta) + np.sin(beta-gamma)), np.cos(alpha)*np.sin(gamma-beta)]])
    return J
...
print("iteration = 0", "qt (degs) =", q0.T/np.pi*180)
# r_goal = np.atleast_2d([0.2, 0.5, -2]).T
r_goal = np.atleast_2d([-1.5, 1.0, -2.5]).T
mag = np.linalg.norm(r_goal)
# print(mag)
r_goal /= mag # unit vector
r_goal *= 2.9
# print(r_goal.T)
...
q_t = q0
tol = 0.1 * np.pi/180 
tol = np.atleast_2d([tol, tol, tol]).T

for i in range(100):
    q = q_t + np.linalg.pinv(J_BF_inB(q_t))@(r_goal-r_BF_inB(q_t))
    q = np.arctan2(np.sin(q), np.cos(q)) # normalize q in [-pi,pi]

    if np.all(np.abs(np.arctan2(np.sin(q-q_t), np.cos(q-q_t))) < tol):
        print("solution converged after", i+1, "iterations")
        break
    q_t = q
    print("iteration =", i+1, "qt (degs) =", q_t.T/np.pi*180)
...
for i in range(len(timeArr)):
    ...
    dq = np.linalg.pinv(J_BF_inB(q)) @ v + (np.eye(3) - np.linalg.pinv(J_BF_inB(q)) @ J_BF_inB(q)) @ dq0
